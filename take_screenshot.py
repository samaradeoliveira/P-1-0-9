    ...
    if results.multi_hand_landmarks:
        for hand_landmark in results.multi_hand_landmarks:
            ...
            if all(finger_fold_status):
                ...
                #image = pyautgui.screenshot()
                #image = cv2.cvtColor(np.aray(image), cv2.COLOR_RGB2BGR)
                #cv2.imwrite("memory_to_disk.png", image)

                # Descomente para salvar a captura de tela diretamente no disco
                # pyautogui.screenshot("straight_to_disk.png")

                # terceiro passo, escrever o código igualzinho está no passo 4 do projeto
                # ele fica dando erro até você escrever corretamente.
